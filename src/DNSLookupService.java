...
public class DNSLookupService {
    ...
    private static int theIndirectionLevel = 0;
    ...
    private static Set<ResourceRecord> getResults(DNSNode node, int indirectionLevel) {
        ...
        // Save indirection level as a global variable
        theIndirectionLevel = indirectionLevel;
        // Check the cache for results of the current node
        Set<ResourceRecord> cacheResults = cache.getCachedResults(node);
        // If cache entry exists then use it to answer query
        if (cacheResults.size() > 0) {
            return cacheResults;
        }
        // Otherwise begin iterative search for node through rootServer
        retrieveResultsFromServer(node, rootServer);
        // Check for results again after completing search
        cacheResults = cache.getCachedResults(node);
        Set<ResourceRecord> finalResults = new HashSet<>();
        for (ResourceRecord cachedRecord : cacheResults) {
            if (cachedRecord.getNode().getType() == RecordType.A ||
                cachedRecord.getNode().getType() == RecordType.AAAA) {
                    finalResults.add(cachedRecord);
            }
        }
        // If any direct results found, return with these results
        if (finalResults.size() > 0) {
            return finalResults;
        } else {
            // Otherwise, begin new query for alias
            DNSNode cnameNode = new DNSNode(node.getHostName(), RecordType.CNAME);
            cacheResults = cache.getCachedResults(cnameNode);
            if (cacheResults.size() == 0) { return Collections.emptySet(); }
            ResourceRecord firstResult = cacheResults.iterator().next();
            DNSNode newNode = new DNSNode(firstResult.getTextResult(), node.getType());
    ...
    private static void queryNextLevel(DNSNode node, Set<ResourceRecord> nameservers) {
        // Check if the cache already contains results for this query
        Set<ResourceRecord> cacheResults = cache.getCachedResults(node);
        // If cache entry exists for query then stop querying
        if (cacheResults.size() > 0) {
            return;
        }
        // If there were no name servers available for the query then stop querying
        if (nameservers.size() == 0) {
            return;
        }
        // Otherwise choose a name server to query next level
        for (ResourceRecord ns : nameservers) {
            DNSNode nsNode = new DNSNode(ns.getTextResult(), RecordType.A);
            cacheResults = cache.getCachedResults(nsNode);
            // Check if the cache already contains results for this query
            if (cacheResults.size() > 0) {
                // If cache entry exists then use it to answer query
                ResourceRecord nsCachedResult = cacheResults.iterator().next();
                retrieveResultsFromServer(node, nsCachedResult.getInetResult());
                return;
            }
        }
        // If no cache entry exists for any name server, then query nameserver
        ResourceRecord nsRecord = nameservers.iterator().next();
        DNSNode nsNode = new DNSNode(nsRecord.getTextResult(), node.getType());
        Set<ResourceRecord> subQueryResults = getResults(nsNode, ++theIndirectionLevel);
        if (subQueryResults.size() == 0) return;
        InetAddress nextServer = subQueryResults.iterator().next().getInetResult();
